  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .set({
            'bio_user_uid': uid,
            'bio_full_name': _nameCtrl.text.trim(),
            'bio_birth_date': Timestamp.fromDate(_birthDate),
            'bio_weight': double.tryParse(_weightCtrl.text) ?? 0,
            'bio_height': double.tryParse(_heightCtrl.text) ?? 0,
            'bio_daily_activity_frequency': _activity,
            'bio_gender': _gender,
            'bio_goal_type': _goal,
            'bio_medical_notes': _notesCtrl.text.trim(),
      ...
      await FirebaseFirestore.instance
          ...
          .doc(uid)
          .update({'user_full_name': _nameCtrl.text.trim()});
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _dropdown(theme, 'Jenis Kelamin', _gender, {
              'male': 'Laki-laki',
              'female': 'Perempuan',
            }, (v) => setState(() => _gender = v!)),
            const SizedBox(height: 16),
            _dropdown(theme, 'Tujuan Fitness', _goal, {
              'cutting': 'Cutting',
              'bulking': 'Bulking',
              'fitness': 'Fitness',
            }, (v) => setState(() => _goal = v!)),
