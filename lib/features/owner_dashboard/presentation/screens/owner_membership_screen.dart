...
class _State extends ConsumerState<OwnerMembershipScreen> {
        ...
        data: (gym) {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(padding: const EdgeInsets.fromLTRB(24, 24, 24, 0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Kelola data member gym kamu', style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurface.withValues(alpha: 0.6))),
            ...
            Expanded(child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.separated(padding: const EdgeInsets.fromLTRB(24, 0, 24, 32), itemCount: docs.length,
  ...
  Stream<QuerySnapshot> _buildQuery(String gymId) {
    var q = FirebaseFirestore.instance.collection('gym_members_registry').where('mem_gym_id', isEqualTo: gymId);
  ...
  Widget _emptyState(ThemeData theme) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    const SizedBox(height: 8),
    Text('Data member akan muncul setelah\nmember bergabung ke gym kamu.', textAlign: TextAlign.center,
  ...
  Widget _memberTile(BuildContext ctx, Map<String, dynamic> d, ThemeData theme) {
    ...
    final status = d['mem_membership_status'] ?? 'inactive';
    ...
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: theme.colorScheme.surface,
      borderRadius: BorderRadius.circular(14), border: Border.all(color: theme.colorScheme.onSurface.withValues(alpha: 0.08))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(d['mem_user_uid'] ?? '', style: theme.textTheme.bodySmall?.copyWith(color: AppColors.lightSecondaryText), maxLines: 1, overflow: TextOverflow.ellipsis),
            Text('${d['mem_membership_type'] ?? 'monthly'}', style: theme.textTheme.bodySmall),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(
            color: (isActive ? AppColors.success : AppColors.error).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(6)),
            child: Text(isActive ? 'Aktif' : 'Tidak Aktif', style: TextStyle(
              color: isActive ? AppColors.success : AppColors.error, fontSize: 11, fontWeight: FontWeight.w600))),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          _stat('Poin', '${d['mem_current_points_balance'] ?? 0}'),
          _stat('Streak', '${d['mem_streak_consecutive_days'] ?? 0}'),
          _stat('Check-in', '${d['mem_total_checkin_count'] ?? 0}'),
        ]),
        const SizedBox(height: 8),
        Text('Periode: $start - $end', style: theme.textTheme.bodySmall?.copyWith(color: AppColors.lightSecondaryText)),
      ]));
  }
