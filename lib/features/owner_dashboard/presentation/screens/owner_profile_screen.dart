    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Center(child: Column(children: [
          ...
          Text(auth.user?.fullName ?? 'Owner', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(
            ...
            child: const Text('Owner Gym', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600, fontSize: 12))),
          const SizedBox(height: 8),
          // Show gym name
          gymAsync.when(
            loading: () => const SizedBox.shrink(),
            error: (error, stackTrace) => const SizedBox.shrink(),
            data: (gym) => gym != null ? Text(gym.gtNameTitle, style: theme.textTheme.bodyMedium?.copyWith(
              fontWeight: FontWeight.w600, color: AppColors.lightSecondaryText)) : const SizedBox.shrink(),
          ),
        ])),
