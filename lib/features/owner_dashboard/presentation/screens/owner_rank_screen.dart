        ...
        data: (gym) {
          ...
          return StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .where('rank_parent_gym_id', isEqualTo: gym.gtIdKey)
                .orderBy('rank_priority_order')
                .snapshots(),
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFFDB2777).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10)),
                        ...
                        Expanded(
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            Text(
                              'Min. ${d['rank_min_points_threshold'] ?? 0} poin • Urutan #${d['rank_priority_order'] ?? 0}',
          ...
          child: SingleChildScrollView(
            child: Form(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(
                  isEdit ? 'Ubah Rank' : 'Tambah Rank',
                  style: Theme.of(ctx).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 20),
                _inp(ctx, 'Nama Rank*', titleCtrl,
                    validator: (v) => (v == null || v.trim().isEmpty) ? 'Nama rank wajib diisi' : null),
                const SizedBox(height: 12),
                _inp(ctx, 'Min. Poin*', pointsCtrl, num: true, validator: (v) {
                  if (v == null || v.trim().isEmpty) return 'Minimum poin wajib diisi';
                  final n = int.tryParse(v.trim());
                  if (n == null || n < 0) return 'Poin harus >= 0';
                  return null;
                }),
                const SizedBox(height: 12),
                _inp(ctx, 'Benefit (pisahkan dengan koma)*', benefitCtrl, maxLines: 3, validator: (v) {
                  if (v == null || v.trim().isEmpty) return 'Daftar benefit wajib diisi';
                  final list = v.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
                  if (list.isEmpty) return 'Masukkan minimal satu benefit';
                  return null;
                }),
                const SizedBox(height: 12),
                _inp(ctx, 'Urutan Prioritas*', orderCtrl, num: true, validator: (v) {
                  if (v == null || v.trim().isEmpty) return 'Urutan prioritas wajib diisi';
                  final n = int.tryParse(v.trim());
                  if (n == null || n < 1) return 'Urutan harus >= 1';
                  return null;
                }),
                    ...
                    child: FilledButton(
                      onPressed: isSaving ? null : () async {
                        ...
                        try {
                          ...
                          final benefits = benefitCtrl.text
                              .split(',')
                              .map((s) => s.trim())
                          ...
                          if (isEdit) {
                            await FirebaseFirestore.instance.collection('gym_master_ranks').doc(docId).update({
                              'rank_title_name': titleCtrl.text.trim(),
                              'rank_min_points_threshold': int.tryParse(pointsCtrl.text.trim()) ?? 0,
                              'rank_benefit_description_list': benefits,
                              'rank_priority_order': int.tryParse(orderCtrl.text.trim()) ?? 0,
                            });
                          } else {
