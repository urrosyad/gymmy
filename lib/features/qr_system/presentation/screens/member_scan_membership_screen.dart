...
class _State extends ConsumerState<MemberScanMembershipScreen> {
  ...
  Future<void> _handleScan(String raw) async {
    ...
    try {
      ...
      final existing = await FirebaseFirestore.instance
          ...
          .get();
      final hasActiveMembership = existing.docs.any((doc) {
        final data = doc.data();
      ...
      if (hasActiveMembership) {
        setState(() {
          _error = 'Anda sudah memiliki membership aktif.';
          _processing = false;
        });
        return;
      }
      ...
      await docRef.set({
        ...
        'mem_membership_end_date': Timestamp.fromDate(
      ...
      await logRef.set({
        ...
        'log_member_id': docRef.id,
        'log_gym_id': gymId,
        'log_category_type': 'membership',
        'log_reference_class_id': '',
        'log_recorded_at': FieldValue.serverTimestamp(),
        'log_user_uid': userId,
        'log_qr_session_id': '',
        'log_validated_by_owner_uid': ownerId,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Membership berhasil diaktifkan selama 30 hari.',
  ...
  Widget _buildScanner(ThemeData theme) => Stack(
    children: [
      MobileScanner(
        onDetect: (c) {
      ...
      Positioned(
        ...
        child: Center(
          child: Container(
            ...
            child: const Text(
              'Arahkan kamera ke QR Membership Gym',
              style: TextStyle(
                color: Colors.white,
